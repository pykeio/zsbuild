...
pub fn as_str<'s>(ptr: *const c_char, len: usize) -> &'s str {
	unsafe { str::from_utf8_unchecked(slice::from_raw_parts(ptr.cast(), len)) }
}

pub fn as_str_opt<'s>(ptr: *const c_char, len: usize) -> Option<&'s str> {
	(!ptr.is_null()).then(|| as_str(ptr, len))
}

pub fn as_str_or_empty<'s>(ptr: *const c_char, len: usize) -> &'s str {
